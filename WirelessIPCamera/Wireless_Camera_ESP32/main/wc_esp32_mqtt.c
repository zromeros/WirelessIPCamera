...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    int msg_id;
    int id_request;
    switch (event->event_id) { //Original
        case MQTT_EVENT_CONNECTED:
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_CONNECTED");
        	//id_request = spi_to_camera_id(PROTOCOL_ASK_ID);
        	camera_id = "12345678123456781234567812345678";
            /*while(1){
        		if(id_request != 0){
        			id_request = spi_to_camera_id(PROTOCOL_ASK_ID);
        		}else{
        	...
        	/* while(strcmp(camera_id,NVS_READ_ERROR)==0){
            	vTaskDelay(5000 / portTICK_RATE_MS);
            }*/
          	mqtt_pub_id(camera_id);
            mqtt_sub_id(camera_id);
            /*if (strcmp(camera_id, NVS_READ_ERROR) == 10){
...
void mqtt_cfg(void)
{
	...
	vTaskDelay(1000 / portTICK_RATE_MS); //Se espera un segundo para recibir el ID
	/*Solo se configura esta conectado al wifi*/
