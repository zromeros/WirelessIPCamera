...
esp_err_t wifi_event_handler(void *ctx, system_event_t *event)
{
    switch(event->event_id) {
		...
		case SYSTEM_EVENT_STA_GOT_IP:
			...
			// variables del estado de conexion true
			connected = true;
		...
		case SYSTEM_EVENT_STA_DISCONNECTED:
			...
			// variables del estado de conexion false
			connected = false;
...
void disconnection_handler(int reason){
	switch(reason){
	//Generar posibles eventos para el manejo de desconexion
		case WIFI_REASON_UNSPECIFIED:
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_UNSPECIFIED");
			esp_wifi_connect();
			break;

		case WIFI_REASON_AUTH_EXPIRE: //2
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_AUTH_EXPIRE");
		...
		case WIFI_REASON_ASSOC_EXPIRE:
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_ASSOC_EXPIRE");
			nvs_clear_credentials();
			xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
			break;

		case WIFI_REASON_NOT_AUTHED:
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_NOT_AUTHED");
			nvs_clear_credentials();
			xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
			break;

		case WIFI_REASON_NOT_ASSOCED: //7
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_NOT_ASSOCEDE");
			nvs_clear_credentials();
			xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
			break;

		case WIFI_REASON_NO_AP_FOUND://201
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_NO_AP_FOUND");
			//nvs_clear_credentials();
			//xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
			esp_wifi_connect();
			break;
		...
		case WIFI_REASON_AUTH_FAIL:
			ESP_LOGI(TAG_WIFI, "WIFI_REASON_AUTH_FAIL");
			nvs_clear_credentials();
			xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
			break;
...
void sc_callback(smartconfig_status_t status, void *pdata)
{
    switch (status) {
        ...
        case SC_STATUS_LINK_OVER:
            ...
            // Configuracion exitosa del SmartConfig
...
void smartconfig_task(void *void_parameters)
{
    ...
    while (1) {
    	//While infinito hasta que el smart config tenga la configuracion exitosa
        uxBits = xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT | ESPTOUCH_DONE_BIT, true, false, portMAX_DELAY);
...
void wifi_cfg(void)
{
    ...
    // Peticion de credenciales wifi
    //char * ssid = nvs_read("ssid");
    //char * password = nvs_read("password");

    /*CREDENCIALES ALTERINFO
     */
    //char * ssid = "ALTERINFO";
    //char * password = "3zcgalterinfo";
    //char * ssid = "PTS_CENTRAL";
    //char * password = "ababababab";
    ...
    if (strcmp(ssid, NVS_READ_ERROR) == 0 || strcmp(password, NVS_READ_ERROR) == 0){
    	// iniciar SmartConfig como tarea en el FREERTOS
//GUIA: xTaskCreate( vTaskCode, "NAME", STACK_SIZE, &ucParameterToPass, tskIDLE_PRIORITY, &xHandle );
    	xTaskCreate(&smartconfig_task, "smartconfig_task", 2048, NULL, 5, &smartconfig_task_handler);
    }
    else{
